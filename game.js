...
// ===============================
//  バッジ・実績
//  ※ BADGE_DEFS / TITLE_DEFS / PET_STAGES は js/data.js で定義
// ===============================
...
// ===============================
//  たまごそだて（ロジック）
//  ※ PET_STAGES の定義は js/data.js を参照
// ===============================
...
function renderReadingPage() {
  ...
  const sentencesHTML = t.sentences.map((s, si) => {
    ...
    const wordSpans = words.map(w => {
      ...
      if (meaning) {
        return `<span class="tappable-word" onclick="showWordPop('${clean}','${meaning}')">${w}</span>`;
      }
      return `<span>${w}</span>`;
    }).join('');
    return `
...
function speakAll() {
  if (!readingText) return;
  const full = readingText.sentences.join(' ');
  speak(full, 0.8);
}
...
function renderRepeatStep() {
  ...
  if (repeatIndex >= sentences.length) {
    renderRepeatComplete();
    return;
  }

  const s = sentences[repeatIndex];
  ...
  document.getElementById('app').innerHTML = `
    <div class="game-screen">
      ...
      <div class="choices-grid">
        ${q.choices.map(c => `
          <button class="choice-btn" data-word="${c}" onclick="checkBlank('${c}', this)">
...
function checkBlank(chosen, btn) {
  ...
  if (isCorrect) {
    readingScore++;
    speak(chosen);
    showFeedback(true, `せいかい！「${chosen}」🎉`);
  } else {
    btn.classList.add('wrong');
    speak(q.answer, 0.75);
    showFeedback(false, `こたえは「${q.answer}」だよ！`);
  }
  readingIndex++;
...
function renderContentQuestion() {
  ...
  document.getElementById('app').innerHTML = `
    <div class="game-screen">
      ...
      <div class="choices-grid">
        ${q.choices.map(c => `
          <button class="choice-btn" data-ans="${c}" onclick="checkContentQ('${c}', this)">
            <span class="choice-text">${c}</span>
          </button>
        `).join('')}
      </div>
